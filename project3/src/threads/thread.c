               ...
               thread_func *function, void *aux) 
{
  ...
  t->fdt = palloc_get_multiple(PAL_ZERO, FDT_PAGES);  //allocate fdt 
    if (t->fdt == NULL) 
        return TID_ERROR;
  /* Initialize thread. */
...
thread_awake (int64_t ticks)
{
  ...
  while (e != list_end (&sleep_list)){
    ...
    if (current_thread->wakeup <= ticks){	
      e = list_remove (e); 
...
void 
thread_evaluate_preemption(void)
{
     ...
     if (!list_empty(&ready_list)) {
         ...
         if (current_thread->priority < first_ready_thread->priority) {
...
donate_priority(void)
{
    ...
    for (int depth = 0; depth < depth_limit; depth++){
        ...
        if (!cur->wait_lock) 
            break;
        struct thread *holder_thread = cur->wait_lock->holder;
        holder_thread->priority = cur->priority;
...
void 
remove_donations_lock(struct lock *target_lock)
{
    struct thread *cur = thread_current();
    ...
    for (struct list_elem *e = list_begin(&cur->donations); 
         ...
         e = list_next(e)) {
        struct thread *donating_thread = list_entry(e, struct thread, donation_elem);

        
        if (donating_thread->wait_lock == target_lock) {
...
refresh_priority(void)
{
    ...
    if (!list_empty(&cur->donations)) {
        ...
        struct thread *highest_donating_thread = list_entry(list_front(&cur->donations), struct thread, donation_elem);

        
        if (highest_donating_thread->priority > cur->priority)
            cur->priority = highest_donating_thread->priority;
    }
}
