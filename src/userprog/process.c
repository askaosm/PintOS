...
int process_add_file(struct file *fileP); //project2: add the file to the fdt of current thread


void parse_filename(char *file_name, char *cmd_name);
void parse_filename(char *file_name, char *cmd_name){
   char *save_ptr;
  char *token = strtok_r(file_name, " ", &save_ptr);
    if (token != NULL) {
        
        strlcpy(cmd_name, token, strlen(token) + 1);
    }
}
...
start_process (void *file_name_)
{
  ...
  success = load (file_name, &if_.eip, &if_.esp);  //cmd??
...
int process_add_file(struct file *fileP) {
...
file *process_get_file(int fd){
   ...
   if (fd<2) return NULL;
   if (fd>=FDT_COUNT_LIMIT) return NULL;
   return fdt[fd];
}
...
/*tid_t
process_fork(const char *name, struct intr_frame *if_UNUSED) {
    struct thread *cur = thread_current();
...
//  strncpy(fn_copy,file_name,strlen(file_name)+1);
...
//////////////////////////////////////////////////////
  ...
  if(argv==NULL){
    printf("Error: memory allocoation\n");
    goto done;
  }
  int argc=0;
  for(token=file_name; token!=NULL; token=strtok_r(NULL, " ", &save_ptr)){
   ...
   ///push argument and push address of argument adn push argv , argc
   // push argument  //reverse
  for (int i = argc - 1; i >= 0; --i) {
    *esp -= strlen(argv[i]) + 1;
    argv[i] = memcpy(*esp, argv[i], strlen(argv[i]) + 1);
...
//   hex_dump((int)*esp, *esp, PHYS_BASE-(int)*esp, true); //PHYS_BASE....dump size
